 ...
 *
 * <p>The class encapsulates all the parameters of the %MSER extraction algorithm (see [wiki
 * article](http://en.wikipedia.org/wiki/Maximally_stable_extremal_regions)).
 *
 * <ul>
 *   <li>there are two different implementation of %MSER: one for grey image, one for color image
 * </ul>
 *
 * <ul>
 *   <li>the grey image algorithm is taken from: CITE: nister2008linear ; the paper claims to be
 *       faster than union-find method; it actually get 1.5~2m/s on my centrino L7200 1.2GHz laptop.
 * </ul>
 *
 * <ul>
 *   <li>the color image algorithm is taken from: CITE: forssen2007maximally ; it should be much
 *       slower than grey image method ( 3~4 times ); the chi_table.h file is taken directly from
 *       paper's source code which is distributed under GPL.
 * </ul>
 *
 * <ul>
 *   <li>(Python) A complete example showing the use of the %MSER detector can be found at
...
public class MSER extends Feature2D {
  ...
  //
  // C++: static Ptr_MSER cv::MSER::create(int _delta = 5, int _min_area = 60, int _max_area =
  // 14400, double _max_variation = 0.25, double _min_diversity = .2, int _max_evolution = 200,
  // double _area_threshold = 1.01, double _min_margin = 0.003, int _edge_blur_size = 5)
  //
   ...
   * @param _min_diversity for color image, trace back to cut off mser with diversity less than
   *     min_diversity
  ...
  public static MSER create(
      ...
      int _max_evolution) {
    return MSER.__fromPtr__(
        create_3(_delta, _min_area, _max_area, _max_variation, _min_diversity, _max_evolution));
  }
  ...
  private static native long create_5(
      int _delta, int _min_area, int _max_area, double _max_variation);
  ...
  // C++:  void cv::MSER::detectRegions(Mat image, vector_vector_Point& msers, vector_Rect& bboxes)
  private static native void detectRegions_0(
      long nativeObj, long image_nativeObj, long msers_mat_nativeObj, long bboxes_mat_nativeObj);
