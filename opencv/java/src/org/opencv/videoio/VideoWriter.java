 ...
 * Video writer class.
 *
 * <p>The class provides C++ API for writing video files or image sequences.
 */
public class VideoWriter {
  ...
  //
  // C++:   cv::VideoWriter::VideoWriter(String filename, int apiPreference, int fourcc, double fps,
  // Size frameSize, bool isColor = true)
  //

  /**
   * The {@code apiPreference} parameter allows to specify API backends to use. Can be used to
   * enforce a specific reader implementation if multiple are available: e.g. cv::CAP_FFMPEG or
   * cv::CAP_GSTREAMER.
   *
   ...
   */
  public VideoWriter(
      String filename, int apiPreference, int fourcc, double fps, Size frameSize, boolean isColor) {
    nativeObj =
        VideoWriter_0(
            filename, apiPreference, fourcc, fps, frameSize.width, frameSize.height, isColor);
  }
  ...
  public VideoWriter(String filename, int apiPreference, int fourcc, double fps, Size frameSize) {
    nativeObj =
        VideoWriter_1(filename, apiPreference, fourcc, fps, frameSize.width, frameSize.height);
  }

  //
  // C++:   cv::VideoWriter::VideoWriter(String filename, int fourcc, double fps, Size frameSize,
  // bool isColor = true)
  //
   ...
   *     VideoWriter::fourcc('M','J','P','G') is a motion-jpeg codec etc. List of codes can be
   *     obtained at [Video Codecs by FOURCC](http://www.fourcc.org/codecs.php) page. FFMPEG backend
   *     with MP4 container natively uses other values as fourcc code: see
   *     [ObjectType](http://www.mp4ra.org/codecs.html), so you may receive a warning message from
   *     OpenCV about fourcc code conversion.
   ...
   *     <p><b>Tips</b>:
   *     <ul>
   *       <li>With some backends {@code fourcc=-1} pops up the codec selection dialog from the
   *           system.
   *       <li>To save image sequence use a proper filename (eg. {@code img_%02d.jpg}) and {@code
   *           fourcc=0} OR {@code fps=0}. Use uncompressed image format (eg. {@code img_%02d.BMP})
   *           to save raw frames.
   *       <li>Most codecs are lossy. If you want lossless video file you need to use a lossless
   *           codecs (eg. FFMPEG FFV1, Huffman HFYU, Lagarith LAGS, etc...)
   *       <li>If FFMPEG is enabled, using {@code codec=0; fps=0;} you can create an uncompressed
   *           (raw) video file.
   *     </ul>
   */
   ...
   *     OpenCV about fourcc code conversion.
   * @param fps Framerate of the created video stream.
   * @param frameSize Size of the video frames. will work with grayscale frames (the flag is
   ...
   *
   * <p>The constructors/functions initialize video writers.
   *
   * <ul>
   *   <li>On Linux FFMPEG is used to write videos;
   *   <li>On Windows FFMPEG or MSWF or DSHOW is used;
   *   <li>On MacOSX AVFoundation is used.
   * </ul>
   */
  public VideoWriter() {
  ...
  //
  // C++:  bool cv::VideoWriter::open(String filename, int apiPreference, int fourcc, double fps,
  // Size frameSize, bool isColor = true)
  //
  ...
  public boolean open(String filename, int apiPreference, int fourcc, double fps, Size frameSize) {
    return open_1(
        nativeObj, filename, apiPreference, fourcc, fps, frameSize.width, frameSize.height);
  }

  //
  // C++:  bool cv::VideoWriter::open(String filename, int fourcc, double fps, Size frameSize, bool
  // isColor = true)
  //
   ...
   *     <p>The method first calls VideoWriter::release to close the already opened file.
   * @param filename automatically generated
   * @param fourcc automatically generated
   * @param fps automatically generated
   * @param frameSize automatically generated
   ...
   * @return {@code true} if the property is supported by the backend used by the VideoWriter
   *     instance.
   */
   ...
   *     <p>The function/method writes the specified image to video file. It must have the same size
  ...
  private static native long VideoWriter_0(
      ...
      int fourcc,
      double fps,
      double frameSize_width,
      double frameSize_height,
      boolean isColor);

  private static native long VideoWriter_1(
      String filename,
      int apiPreference,
      int fourcc,
      double fps,
  ...
  private static native long VideoWriter_3(
      String filename, int fourcc, double fps, double frameSize_width, double frameSize_height);
  ...
  // C++:  bool cv::VideoWriter::open(String filename, int apiPreference, int fourcc, double fps,
  // Size frameSize, bool isColor = true)
  private static native boolean open_0(
      long nativeObj,
      String filename,
      int apiPreference,
      int fourcc,
  ...
  private static native boolean open_1(
      long nativeObj,
      String filename,
      int apiPreference,
      int fourcc,
  ...
  // C++:  bool cv::VideoWriter::open(String filename, int fourcc, double fps, Size frameSize, bool
  // isColor = true)
  private static native boolean open_2(
      long nativeObj,
      String filename,
      int fourcc,
      double fps,
  ...
  private static native boolean open_3(
      long nativeObj,
      String filename,
      int fourcc,
      double fps,
