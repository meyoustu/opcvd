 ...
 *
 * <p>Model allows to set params for preprocessing input image. Model creates net from file with
...
public class Model extends Net {
  ...
  /**
   * Create model from deep learning network represented in one of the supported formats. An order
   * of {@code model} and {@code config} arguments does not matter.
   *
   ...
   *
   * @param size New input size. <b>Note:</b> If shape of the new blob less than 0, then frame size
   *     not change.
   * @return automatically generated
   */
   ...
   * @param height New input height. <b>Note:</b> If shape of the new blob less than 0, then frame
   *     size not change.
   * @return automatically generated
   */
  ...
  //
  // C++:  void cv::dnn::Model::setInputParams(double scale = 1.0, Size size = Size(), Scalar mean =
  // Scalar(), bool swapRB = false, bool crop = false)
  //
   ...
   * @param scale Multiplier for frame values.
   * @param swapRB Flag which indicates that swap first and last channels.
   * @param crop Flag which indicates whether image will be cropped after resize or not. blob(n, c,
   *     y, x) = scale * resize( frame(y, x, c) ) - mean(c) )
   */
   ...
   * @param mean Scalar with mean values which are subtracted from channels.
   * @param scale Multiplier for frame values.
   * @param swapRB Flag which indicates that swap first and last channels. blob(n, c, y, x) = scale
   *     * resize( frame(y, x, c) ) - mean(c) )
   */
   ...
   * @param size New input size.
   * @param mean Scalar with mean values which are subtracted from channels.
   * @param scale Multiplier for frame values. blob(n, c, y, x) = scale * resize( frame(y, x, c) ) -
   *     mean(c) )
   */
  ...
  /**
   * Set preprocessing parameters for frame. blob(n, c, y, x) = scale * resize( frame(y, x, c) ) -
   * mean(c) )
   */
  ...
  // C++:  Model cv::dnn::Model::setInputMean(Scalar mean)
  private static native long setInputMean_0(
      long nativeObj, double mean_val0, double mean_val1, double mean_val2, double mean_val3);
  ...
  // C++:  void cv::dnn::Model::predict(Mat frame, vector_Mat& outs)
  private static native void predict_0(
      long nativeObj, long frame_nativeObj, long outs_mat_nativeObj);

  // C++:  void cv::dnn::Model::setInputParams(double scale = 1.0, Size size = Size(), Scalar mean =
  // Scalar(), bool swapRB = false, bool crop = false)
  private static native void setInputParams_0(
      long nativeObj,
      double scale,
      double size_width,
      double size_height,
  ...
  private static native void setInputParams_1(
      long nativeObj,
      double scale,
      double size_width,
      double size_height,
  ...
  private static native void setInputParams_2(
      long nativeObj,
      double scale,
      double size_width,
      double size_height,
  ...
  private static native void setInputParams_3(
      long nativeObj, double scale, double size_width, double size_height);
