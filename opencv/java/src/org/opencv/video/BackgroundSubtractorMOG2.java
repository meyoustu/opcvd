 ...
 *
 * <p>The class implements the Gaussian mixture model background subtraction described in CITE:
 * Zivkovic2004 and CITE: Zivkovic2006 .
 */
public class BackgroundSubtractorMOG2 extends BackgroundSubtractor {
   ...
   *
   * <p>If a foreground pixel keeps semi-constant value for about backgroundRatio\*history frames,
   * it's considered background and added to the model as a center of a new component. It
   * corresponds to TB parameter in the paper.
   *
   * @return automatically generated
   */
   ...
   *
   * <p>This parameter defines the number of samples needed to accept to prove the component exists.
   * CT=0.05 is a default value for all the samples. By setting CT=0 you get an algorithm very
   * similar to the standard Stauffer&amp;Grimson algorithm.
   *
   * @return automatically generated
   */
   ...
   * that if a pixel is more than twice darker then it is not shadow. See Prati, Mikic, Trivedi and
   * Cucchiara, Detecting Moving Shadows...*, IEEE PAMI,2003.
   *
   * @return automatically generated
   */
   ...
   *
   * <p>The main threshold on the squared Mahalanobis distance to decide if the sample is well
  ...
  /**
   * Returns the variance threshold for the pixel-model match used for new mixture component
   * generation
   *
   * <p>Threshold for the squared Mahalanobis distance that helps decide when a sample is close to
   * the existing components (corresponds to Tg in the paper). If a pixel is not close to any
   * component, it is considered foreground or added as a new component. 3 sigma =&gt; Tg=3\*3=9 is
   * default. A smaller Tg value generates more components. A higher Tg value may result in a small
   ...
   *
   * <p>Shadow value is the value used to mark shadows in the foreground mask. Default value is 127.
  ...
  //
  // C++:  void cv::BackgroundSubtractorMOG2::apply(Mat image, Mat& fgmask, double learningRate =
  // -1)
  //
   ...
   *
   * @param image Next video frame. Floating point frame will be used without scaling and should be
   *     in range \([0,255]\).
   * @param fgmask The output foreground mask as an 8-bit binary image.
   * @param learningRate The value between 0 and 1 that indicates how fast the background model is
   *     learnt. Negative parameter value makes the algorithm to use some automatically chosen
   *     learning rate. 0 means that the background model is not updated at all, 1 means that the
   ...
   * @param image Next video frame. Floating point frame will be used without scaling and should be
   *     in range \([0,255]\).
   * @param fgmask The output foreground mask as an 8-bit binary image. learnt. Negative parameter
   *     value makes the algorithm to use some automatically chosen learning rate. 0 means that the
   *     background model is not updated at all, 1 means that the background model is completely
  ...
  // C++:  void cv::BackgroundSubtractorMOG2::apply(Mat image, Mat& fgmask, double learningRate =
  // -1)
  private static native void apply_0(
      long nativeObj, long image_nativeObj, long fgmask_nativeObj, double learningRate);
