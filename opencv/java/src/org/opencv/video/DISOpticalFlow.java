 ...
 * about the algorithm can be found at CITE: Kroeger2016 . Includes three presets with preselected
 * parameters to provide reasonable trade-off between speed and quality. However, even the slowest
 * preset is still relatively fast, use DeepFlow if you need better quality and don't care about
 * speed.
 *
 * <p>This implementation includes several additional features compared to the algorithm described
 * in the paper, including spatial propagation of flow vectors (REF: getUseSpatialPropagation), as
 * well as an option to utilize an initial flow approximation passed to REF: calc (which is,
 * essentially, temporal propagation, if the previous frame's flow field is passed).
 */
public class DISOpticalFlow extends DenseOpticalFlow {
  ...
  //
  // C++: static Ptr_DISOpticalFlow cv::DISOpticalFlow::create(int preset =
  ...
  /**
   * Whether to use mean-normalization of patches when computing patch distance. It is turned on by
   * default as it typically provides a noticeable quality boost because of increased robustness to
   * illumination variations. Turn it off if you are certain that your sequence doesn't contain any
   * changes in illumination. SEE: setUseMeanNormalization
   *
   * @return automatically generated
   */
   ...
   * introduced by the coarse-to-fine scheme employed by the DIS optical flow algorithm. Turning
   * this option off can make the output flow field a bit smoother, however. SEE:
  ...
  /**
   * Finest level of the Gaussian pyramid on which the flow is computed (zero level corresponds to
   * the original image resolution). The final flow is obtained by bilinear upscaling. SEE:
   * setFinestScale
   *
   * @return automatically generated
   */
   ...
   * may improve quality in some cases. SEE: setGradientDescentIterations
   *
   * @return automatically generated
   */
  ...
  /**
   * Size of an image patch for matching (in pixels). Normally, default 8x8 patches work well enough
   * in most cases. SEE: setPatchSize
   *
   * @return automatically generated
   */
   ...
   * higher flow quality. SEE: setPatchStride
   *
   * @return automatically generated
   */
   ...
   * high-quality flow. SEE: setGradientDescentIterations
   *
   * @return automatically generated
   */
