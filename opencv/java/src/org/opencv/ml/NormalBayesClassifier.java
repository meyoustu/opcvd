...
public class NormalBayesClassifier extends StatModel {
  ...
  //
  // C++: static Ptr_NormalBayesClassifier cv::ml::NormalBayesClassifier::load(String filepath,
   ...
   *
   * <p>Use NormalBayesClassifier::save to serialize and store an NormalBayesClassifier to disk.
   * Load the NormalBayesClassifier from this file again, by calling this function with the path to
   * the file. Optionally specify the node for the file containing the classifier
   *
  ...
  //
  // C++:  float cv::ml::NormalBayesClassifier::predictProb(Mat inputs, Mat& outputs, Mat&
  // outputProbs, int flags = 0)
  //

  /**
   * Predicts the response for sample(s).
   *
   * <p>The method estimates the most probable classes for input vectors. Input vectors (one or
   * more) are stored as rows of the matrix inputs. In case of multiple input vectors, there should
   * be one output vector outputs. The predicted class for a single input vector is returned by the
   * method. The vector outputProbs contains the output probabilities corresponding to each element
   * of result.
   *
  ...
  public float predictProb(Mat inputs, Mat outputs, Mat outputProbs, int flags) {
    return predictProb_0(
        nativeObj, inputs.nativeObj, outputs.nativeObj, outputProbs.nativeObj, flags);
  }
  ...
  // C++:  float cv::ml::NormalBayesClassifier::predictProb(Mat inputs, Mat& outputs, Mat&
  // outputProbs, int flags = 0)
  private static native float predictProb_0(
      long nativeObj,
      long inputs_nativeObj,
      long outputs_nativeObj,
      long outputProbs_nativeObj,
      int flags);

  private static native float predictProb_1(
      long nativeObj, long inputs_nativeObj, long outputs_nativeObj, long outputProbs_nativeObj);
