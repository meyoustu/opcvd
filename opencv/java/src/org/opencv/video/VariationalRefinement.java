 ...
 *
 * <p>This class implements variational refinement of the input flow field, i.e. it uses input flow
 * to initialize the minimization of the following functional: \(E(U) = \int_{\Omega} \delta
 * \Psi(E_I) + \gamma \Psi(E_G) + \alpha \Psi(E_S) \), where \(E_I,E_G,E_S\) are color constancy,
 * gradient constancy and smoothness terms respectively. \(\Psi(s^2)=\sqrt{s^2+\epsilon^2}\) is a
 * robust penalizer to limit the influence of outliers. A complete formulation and a description of
 * the minimization procedure can be found in CITE: Brox2004
 */
public class VariationalRefinement extends DenseOpticalFlow {
  ...
  /**
   * Number of outer (fixed-point) iterations in the minimization procedure. SEE:
  ...
  /**
   * Number of inner successive over-relaxation (SOR) iterations in the minimization procedure to
   * solve the respective linear system. SEE: setSorIterations
   *
   * @return automatically generated
   */
  ...
  // C++:  void cv::VariationalRefinement::calcUV(Mat I0, Mat I1, Mat& flow_u, Mat& flow_v)
  private static native void calcUV_0(
      long nativeObj,
      long I0_nativeObj,
      long I1_nativeObj,
      long flow_u_nativeObj,
      long flow_v_nativeObj);
