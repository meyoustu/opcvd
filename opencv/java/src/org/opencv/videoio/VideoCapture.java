 ...
 *
 * <p>The class provides C++ API for capturing video from cameras or for reading video files and
 * image sequences.
 *
 * <p>Here is how the class can be used: INCLUDE: samples/cpp/videocapture_basic.cpp
 *
 * <p><b>Note:</b> In REF: videoio_c "C API" the black-box structure {@code CvCapture} is used
 * instead of %VideoCapture. <b>Note:</b>
 *
 * <ul>
 *   <li>(C++) A basic sample on using the %VideoCapture interface can be found at {@code
 *       OPENCV_SOURCE_CODE/samples/cpp/videocapture_starter.cpp}
 *   <li>(Python) A basic sample on using the %VideoCapture interface can be found at {@code
 *       OPENCV_SOURCE_CODE/samples/python/video.py}
 *   <li>(Python) A multi threaded video processing sample can be found at {@code
 *       OPENCV_SOURCE_CODE/samples/python/video_threaded.py}
 *   <li>(Python) %VideoCapture sample showcasing some features of the Video4Linux2 backend {@code
...
public class VideoCapture {
  ...
  /**
   * Default constructor <b>Note:</b> In REF: videoio_c "C API", when you finished working with
   * video, release CvCapture structure with cvReleaseCapture(), or use Ptr&lt;CvCapture&gt; that
   ...
   *
   * <p><b>Note:</b> Stream should be opened.
   *
   ...
   *     <p>The method/function grabs the next frame from video file or camera and returns true
   *     (non-zero) in the case of success.
   *     <p>The primary use of the function is in multi-camera environments, especially when the
   *     cameras do not have hardware synchronization. That is, you call VideoCapture::grab() for
   *     each camera and after that call the slower method VideoCapture::retrieve() to decode and
   *     get frame from each camera. This way the overhead on demosaicing or motion jpeg
   *     decompression etc. is eliminated and the retrieved frames from different cameras will be
   *     closer in time.
   *     <p>Also, when a connected camera is multi-head (for example, a stereo camera or a Kinect
   *     device), the correct way of retrieving data from it is to call VideoCapture::grab() first
   *     and then call VideoCapture::retrieve() one or more times with different values of the
   *     channel parameter.
   *     <p>REF: tutorial_kinect_openni
   */
  public boolean grab() {
   ...
   *
   * <p>If the previous call to VideoCapture constructor or VideoCapture::open() succeeded, the
   * method returns true.
   *
   * @return automatically generated
   */
   ...
   *
   * <p>Parameters are same as the constructor VideoCapture(const String&amp; filename, int
   * apiPreference = CAP_ANY)
   *
   * @return {@code true} if the file has been successfully opened
   *     <p>The method first calls VideoCapture::release to close the already opened file or camera.
   ...
   *
   * <p>Parameters are same as the constructor VideoCapture(int index, int apiPreference = CAP_ANY)
   *
   ...
   *     <p>The method/function combines VideoCapture::grab() and VideoCapture::retrieve() in one
   *     call. This is the most convenient method for reading video files or capturing data from
   *     decode and returns the just grabbed frame. If no frames has been grabbed (camera has been
   *     disconnected, or there are no more frames in video file), the method returns false and the
   *     function returns empty image (with %cv::Mat, test it with Mat::empty()).
   *     <p><b>Note:</b> In REF: videoio_c "C API", functions cvRetrieveFrame() and
   *     cv.RetrieveFrame() return image stored inside the video capturing structure. It is not
   *     allowed to modify or release the image! You can copy the frame using cvCloneImage and then
   ...
   *     (camera has been disconnected, or there are no more frames in video file), the method
   *     returns false and the function returns an empty image (with %cv::Mat, test it with
   *     Mat::empty()).
   *     <p>SEE: read()
   *     <p><b>Note:</b> In REF: videoio_c "C API", functions cvRetrieveFrame() and
   *     cv.RetrieveFrame() return image stored inside the video capturing structure. It is not
   ...
   *     <b>Note:</b> Even if it returns {@code true} this doesn't ensure that the property value
   ...
   *     <p><b>Note:</b> Reading / writing properties involves many layers. Some unexpected result
   *     might happens along this chain. <code>
   *     VideoCapture -&gt; API Backend -&gt; Operating System -&gt; Device Driver -&gt; Device Hardware
   *     </code> The returned value might be different from what really used by the device or it
   *     could be encoded using device dependent rules (eg. steps or percentage). Effective
   *     behaviour depends from device driver and API Backend
   */
   ...
   * destructor.
   *
   * <p>The C function also deallocates memory and clears \*capture pointer.
   */
  public void release() {
    release_0(nativeObj);
  }
   ...
   *
   * <p>methods raise exceptions if not successful instead of returning an error code
   *
