...
public class SVM extends StatModel {
   ...
   *
   * <p>The method returns all the support vectors as a floating-point matrix, where support vectors
   ...
   *
   * <p>The method returns all the uncompressed support vectors of a linear %SVM that the compressed
   ...
   *     <p>The function generates a grid pointer for the specified parameter of the %SVM algorithm.
  ...
  /**
   * Creates empty model. Use StatModel::train to train the model. Since %SVM has several
   * parameters, you may want to find the best parameters for your problem, it can be done with
   * SVM::trainAuto.
   *
   * @return automatically generated
   */
   ...
   *
   * <p>Use SVM::save to serialize and store an SVM to disk. Load the SVM from this file again, by
   * calling this function with the path to the file.
   *
  ...
  //
  // C++:  bool cv::ml::SVM::trainAuto(Mat samples, int layout, Mat responses, int kFold = 10,
  // Ptr_ParamGrid Cgrid = SVM::getDefaultGridPtr(SVM::C), Ptr_ParamGrid gammaGrid =
  // SVM::getDefaultGridPtr(SVM::GAMMA), Ptr_ParamGrid pGrid = SVM::getDefaultGridPtr(SVM::P),
  // Ptr_ParamGrid nuGrid = SVM::getDefaultGridPtr(SVM::NU), Ptr_ParamGrid coeffGrid =
  // SVM::getDefaultGridPtr(SVM::COEF), Ptr_ParamGrid degreeGrid =
  // SVM::getDefaultGridPtr(SVM::DEGREE), bool balanced = false)
  //
   ...
   *     <p>The method trains the %SVM model automatically by choosing the optimal parameters C,
   *     gamma, p, nu, coef0, degree. Parameters are considered optimal when the cross-validation
   *     estimate of the test set error is minimal.
   *     <p>This function only makes use of SVM::getDefaultGrid for parameter optimization and thus
   *     only offers rudimentary parameter options.
   *     <p>This function works for the classification (SVM::C_SVC or SVM::NU_SVC) as well as for
   *     the regression (SVM::EPS_SVR or SVM::NU_SVR). If it is SVM::ONE_CLASS, no optimization is
   *     made and the usual %SVM with parameters specified in params is executed.
   * @return automatically generated
   */
  public boolean trainAuto(
      Mat samples,
      int layout,
      Mat responses,
      int kFold,
    ...
    return trainAuto_0(
        ...
        responses.nativeObj,
        kFold,
        Cgrid.getNativeObjAddr(),
        gammaGrid.getNativeObjAddr(),
        pGrid.getNativeObjAddr(),
        nuGrid.getNativeObjAddr(),
        coeffGrid.getNativeObjAddr(),
        degreeGrid.getNativeObjAddr(),
        balanced);
  }
   ...
   * @param nuGrid grid for nu
   * @param coeffGrid grid for coeff
   * @param degreeGrid grid for degree balanced cross-validation subsets that is proportions between
   *     classes in subsets are close to such proportion in the whole train dataset.
   *     <p>The method trains the %SVM model automatically by choosing the optimal parameters C,
   *     gamma, p, nu, coef0, degree. Parameters are considered optimal when the cross-validation
   *     estimate of the test set error is minimal.
   *     <p>This function only makes use of SVM::getDefaultGrid for parameter optimization and thus
   ...
   * @param pGrid grid for p
   * @param nuGrid grid for nu
   * @param coeffGrid grid for coeff balanced cross-validation subsets that is proportions between
   *     classes in subsets are close to such proportion in the whole train dataset.
   *     <p>The method trains the %SVM model automatically by choosing the optimal parameters C,
   *     gamma, p, nu, coef0, degree. Parameters are considered optimal when the cross-validation
   *     estimate of the test set error is minimal.
   ...
   * @param gammaGrid grid for gamma
   * @param pGrid grid for p
   * @param nuGrid grid for nu balanced cross-validation subsets that is proportions between classes
   *     in subsets are close to such proportion in the whole train dataset.
   *     <p>The method trains the %SVM model automatically by choosing the optimal parameters C,
   *     gamma, p, nu, coef0, degree. Parameters are considered optimal when the cross-validation
   *     estimate of the test set error is minimal.
   *     <p>This function only makes use of SVM::getDefaultGrid for parameter optimization and thus
   ...
   * @param Cgrid grid for C
   * @param gammaGrid grid for gamma
   * @param pGrid grid for p balanced cross-validation subsets that is proportions between classes
   *     in subsets are close to such proportion in the whole train dataset.
   *     <p>The method trains the %SVM model automatically by choosing the optimal parameters C,
   *     gamma, p, nu, coef0, degree. Parameters are considered optimal when the cross-validation
   *     estimate of the test set error is minimal.
   ...
   *     subset is used to test the model, the others form the train set. So, the %SVM algorithm is
   * @param Cgrid grid for C
   * @param gammaGrid grid for gamma balanced cross-validation subsets that is proportions between
   *     classes in subsets are close to such proportion in the whole train dataset.
   *     <p>The method trains the %SVM model automatically by choosing the optimal parameters C,
   *     gamma, p, nu, coef0, degree. Parameters are considered optimal when the cross-validation
   *     estimate of the test set error is minimal.
   ...
   */
  public boolean trainAuto(
      Mat samples, int layout, Mat responses, int kFold, ParamGrid Cgrid, ParamGrid gammaGrid) {
    return trainAuto_5(
        nativeObj,
        samples.nativeObj,
        layout,
   ...
   * @param kFold Cross-validation parameter. The training set is divided into kFold subsets. One
   *     subset is used to test the model, the others form the train set. So, the %SVM algorithm is
   * @param Cgrid grid for C balanced cross-validation subsets that is proportions between classes
   *     in subsets are close to such proportion in the whole train dataset.
   *     <p>The method trains the %SVM model automatically by choosing the optimal parameters C,
   *     gamma, p, nu, coef0, degree. Parameters are considered optimal when the cross-validation
   *     estimate of the test set error is minimal.
  ...
  public boolean trainAuto(Mat samples, int layout, Mat responses, int kFold, ParamGrid Cgrid) {
    return trainAuto_6(
        nativeObj, samples.nativeObj, layout, responses.nativeObj, kFold, Cgrid.getNativeObjAddr());
  }
   ...
   * @param samples training samples
   * @param layout See ml::SampleTypes.
   * @param responses vector of responses associated with the training samples. subset is used to
   *     test the model, the others form the train set. So, the %SVM algorithm is balanced
   *     cross-validation subsets that is proportions between classes in subsets are close to such
   ...
   *     decision functions.
   * @param alpha the optional output vector for weights, corresponding to different support
   *     vectors. In the case of linear %SVM all the alpha's will be 1's.
   * @param svidx the optional output vector of indices of support vectors within the matrix of
   *     support vectors (which can be retrieved by SVM::getSupportVectors). In the case of linear
   *     %SVM each decision function consists of a single "compressed" support vector.
   *     <p>The method returns rho parameter of the decision function, a scalar subtracted from the
   *     weighted sum of kernel responses.
   * @return automatically generated
   */
  ...
  // SVM::getDefaultGridPtr(SVM::COEF), Ptr_ParamGrid degreeGrid =
  // SVM::getDefaultGridPtr(SVM::DEGREE), bool balanced = false)
  private static native boolean trainAuto_0(
      long nativeObj,
      long samples_nativeObj,
      int layout,
      long responses_nativeObj,
      int kFold,
      long Cgrid_nativeObj,
      long gammaGrid_nativeObj,
      long pGrid_nativeObj,
      long nuGrid_nativeObj,
      long coeffGrid_nativeObj,
      long degreeGrid_nativeObj,
      boolean balanced);

  private static native boolean trainAuto_1(
      long nativeObj,
      long samples_nativeObj,
      int layout,
      long responses_nativeObj,
  ...
  private static native boolean trainAuto_2(
      long nativeObj,
      long samples_nativeObj,
      int layout,
      long responses_nativeObj,
  ...
  private static native boolean trainAuto_3(
      long nativeObj,
      long samples_nativeObj,
      int layout,
      long responses_nativeObj,
  ...
  private static native boolean trainAuto_4(
      long nativeObj,
      long samples_nativeObj,
      int layout,
      long responses_nativeObj,
  ...
  private static native boolean trainAuto_5(
      long nativeObj,
      long samples_nativeObj,
      int layout,
      long responses_nativeObj,
  ...
  private static native boolean trainAuto_6(
      long nativeObj,
      long samples_nativeObj,
      int layout,
      long responses_nativeObj,
  ...
  private static native boolean trainAuto_7(
      long nativeObj, long samples_nativeObj, int layout, long responses_nativeObj, int kFold);

  private static native boolean trainAuto_8(
  ...
  // C++:  double cv::ml::SVM::getDecisionFunction(int i, Mat& alpha, Mat& svidx)
  private static native double getDecisionFunction_0(
      long nativeObj, int i, long alpha_nativeObj, long svidx_nativeObj);
