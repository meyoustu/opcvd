...
public class RTrees extends DTrees {
  ...
  /**
   * Returns the variable importance array. The method returns the variable importance vector,
   * computed at the training stage when CalculateVarImportance is set to true. If this flag was set
   * to false, the empty matrix is returned.
   *
  ...
  /**
   * Creates the empty model. Use StatModel::train to train the model, StatModel::train to create
   * and train the model, Algorithm::load to load the pre-trained model.
   *
   * @return automatically generated
   */
   ...
   *
   * <p>Use RTree::save to serialize and store an RTree to disk. Load the RTree from this file
   * again, by calling this function with the path to the file. Optionally specify the node for the
  ...
  /**
   * Returns the result of each individual tree in the forest. In case the model is a regression
   * problem, the method will return each of the trees' results for each of the sample cases. If the
   * model is a classifier, it will return a Mat with samples + 1 rows, where the first row gives
   * the class number and the following rows return the votes each class had for each sample.
   *
  ...
  // C++:  void cv::ml::RTrees::getVotes(Mat samples, Mat& results, int flags)
  private static native void getVotes_0(
