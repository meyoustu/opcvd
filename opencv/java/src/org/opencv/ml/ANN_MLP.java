 ...
 *
 * <p>Unlike many other models in ML that are constructed and trained at once, in the MLP model
 * these steps are separated. First, a network with the specified topology is created using the
 * non-default constructor or the method ANN_MLP::create. All the weights are set to zeros. Then,
 * the network is trained using a set of input and output vectors. The training procedure can be
 * repeated more than once, that is, the weights can be adjusted based on the new training data.
 *
 * <p>Additional flags for StatModel::train are available: ANN_MLP::TrainFlags.
 *
...
public class ANN_MLP extends StatModel {
  ...
  /**
   * Integer vector specifying the number of neurons in each layer including the input and output
   * layers. The very first element specifies the number of elements in the input layer. The last
   * element - number of elements in the output layer. SEE: setLayerSizes
   *
   ...
   * Creates empty model
   *
   * <p>Use StatModel::train to train the model, Algorithm::load&lt;ANN_MLP&gt;(filename) to load
   * the pre-trained model. Note that the train method has optional flags: ANN_MLP::TrainFlags.
   *
   * @return automatically generated
   */
   ...
   *
   * <p>Use ANN::save to serialize and store an ANN to disk. Load the ANN from this file again, by
  ...
  //
  // C++:  void cv::ml::ANN_MLP::setActivationFunction(int type, double param1 = 0, double param2 =
  // 0)
  //

  /**
   * Initialize the activation function for each neuron. Currently the default and the only fully
   * supported activation function is ANN_MLP::SIGMOID_SYM.
   *
   ...
   * Integer vector specifying the number of neurons in each layer including the input and output
   * layers. The very first element specifies the number of elements in the input layer. The last
   * element - number of elements in the output layer. Default value is empty Mat. SEE:
   * getLayerSizes
   *
   ...
   * @param param1 passed to setRpropDW0 for ANN_MLP::RPROP and to setBackpropWeightScale for
   *     ANN_MLP::BACKPROP and to initialT for ANN_MLP::ANNEAL.
  ...
  // C++:  void cv::ml::ANN_MLP::setActivationFunction(int type, double param1 = 0, double param2 =
  // 0)
  private static native void setActivationFunction_0(
      long nativeObj, int type, double param1, double param2);
  ...
  // C++:  void cv::ml::ANN_MLP::setTrainMethod(int method, double param1 = 0, double param2 = 0)
  private static native void setTrainMethod_0(
      long nativeObj, int method, double param1, double param2);
