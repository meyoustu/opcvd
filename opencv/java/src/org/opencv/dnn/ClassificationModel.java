 ...
 *
 * <p>ClassificationModel allows to set params for preprocessing input image. ClassificationModel
 * creates net from file with trained weights and config, sets preprocessing input, runs forward
 * pass and return top-1 prediction.
 */
public class ClassificationModel extends Model {
  ...
  /**
   * Create classification model from network represented in one of the supported formats. An order
  ...
  // C++:  void cv::dnn::ClassificationModel::classify(Mat frame, int& classId, float& conf)
  private static native void classify_0(
      long nativeObj, long frame_nativeObj, double[] classId_out, double[] conf_out);
