...
public class Subdiv2D {
  ...
  /**
   * @param rect Rectangle that includes all of the 2D points that are to be added to the
   *     subdivision.
   *     <p>The function creates an empty Delaunay subdivision where 2D points can be added using
   *     the function insert() . All of the points to be added must be within the specified
   *     rectangle, otherwise a runtime error is raised.
   */
  ...
  /**
   * creates an empty Subdiv2D object. To create a new empty Delaunay subdivision you need to use
   * the #initDelaunay function.
   */
  public Subdiv2D() {
   ...
   *     <p>The function is another function that locates the input point within the subdivision. It
   *     finds the subdivision vertex that is the closest to the input point. It is not necessarily
   *     one of vertices of the facet containing the input point, though the facet (located using
   *     locate() ) is used as a starting point.
   * @return vertex ID.
   */
   ...
   *
   * @param edge Subdivision edge ID.
   * @param nextEdgeType Parameter specifying which of the related edges to return. The following
   *     values are possible:
   *     <ul>
   *       <li>NEXT_AROUND_ORG next around the edge origin ( eOnext on the picture below if e is the
   *           input edge)
   *       <li>NEXT_AROUND_DST next around the edge vertex ( eDnext )
   *       <li>PREV_AROUND_ORG previous around the edge origin (reversed eRnext )
   *       <li>PREV_AROUND_DST previous around the edge destination (reversed eLnext )
   *       <li>NEXT_AROUND_LEFT next around the left facet ( eLnext )
   *       <li>NEXT_AROUND_RIGHT next around the right facet ( eRnext )
   *       <li>PREV_AROUND_LEFT previous around the left facet (reversed eOnext )
   *       <li>PREV_AROUND_RIGHT previous around the right facet (reversed eDnext )
   *     </ul>
   ...
   *     <p>The function inserts a single point into a subdivision and modifies the subdivision
   *     topology appropriately. If a point with the same coordinates exists already, no new point
   *     is added.
   * @return the ID of the point.
   *     <p><b>Note:</b> If the point is outside of the triangulation specified rect a runtime error
   *     is raised.
   */
   ...
   *     <p>The function locates the input point within the subdivision and gives one of the
   *     triangle edges or vertices.
   * @return an integer which specify one of the following five cases for point location:
   *     <ul>
   *       <li>The point falls into some facet. The function returns #PTLOC_INSIDE and edge will
   *           contain one of edges of the facet.
   *       <li>The point falls onto the edge. The function returns #PTLOC_ON_EDGE and edge will
   *           contain this edge.
   *       <li>The point coincides with one of the subdivision vertices. The function returns
   *           #PTLOC_VERTEX and vertex will contain a pointer to the vertex.
   *       <li>The point is outside the subdivision reference rectangle. The function returns
   *           #PTLOC_OUTSIDE_RECT and no pointers are filled.
   *       <li>One of input arguments is invalid. A runtime error is raised or, if silent or
   *           "parent" error processing mode is selected, #PTLOC_ERROR is returned.
   *     </ul>
   */
   ...
   *
   * @param edge Subdivision edge ID.
   * @return an integer which is next edge ID around the edge origin: eOnext on the picture above if
   *     e is the input edge).
   */
   ...
   *
   * @param edge Subdivision edge ID.
   * @param rotate Parameter specifying which of the edges of the same quad-edge as the input one to
   *     return. The following values are possible:
   *     <ul>
   *       <li>0 - the input edge ( e on the picture below if e is the input edge)
   *       <li>1 - the rotated edge ( eRot )
   *       <li>2 - the reversed edge (reversed e (in green))
   *       <li>3 - the reversed rotated edge (reversed eRot (in green))
   *     </ul>
   *
   ...
   *     <p>The function gives each edge as a 4 numbers vector, where each two are one of the edge
   ...
   *     <p>The function gives one edge ID for each triangle.
   */
   ...
   *     <p>The function gives each triangle as a 6 numbers vector, where each two are one of the
   *     triangle vertices. i.e. p1_x = v[0], p1_y = v[1], p2_x = v[2], p2_y = v[3], p3_x = v[4],
   *     p3_y = v[5].
   */
   ...
   *     <p>The function inserts a vector of points into a subdivision and modifies the subdivision
  ...
  // C++:  int cv::Subdiv2D::findNearest(Point2f pt, Point2f* nearestPt = 0)
  private static native int findNearest_0(
      long nativeObj, double pt_x, double pt_y, double[] nearestPt_out);
  ...
  private static native int locate_0(
      long nativeObj, double pt_x, double pt_y, double[] edge_out, double[] vertex_out);
  ...
  // C++:  void cv::Subdiv2D::getLeadingEdgeList(vector_int& leadingEdgeList)
  private static native void getLeadingEdgeList_0(
      long nativeObj, long leadingEdgeList_mat_nativeObj);
  ...
  // C++:  void cv::Subdiv2D::initDelaunay(Rect rect)
  private static native void initDelaunay_0(
      long nativeObj, int rect_x, int rect_y, int rect_width, int rect_height);
