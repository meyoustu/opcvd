...
public class EM extends StatModel {
   ...
   *
   * <p>Returns matrix with the number of rows equal to the number of mixtures and number of columns
   ...
   *
   * <p>Returns vector with the number of elements equal to the number of mixtures.
   *
   * @return automatically generated
   */
  ...
  /**
   * Creates empty %EM model. The model should be trained then using StatModel::train(traindata,
   * flags) method. Alternatively, you can use one of the EM::train\* methods or load it from file
   * using Algorithm::load&lt;EM&gt;(filename).
   *
   * @return automatically generated
   */
   ...
   *
   * <p>Use EM::save to serialize and store an EM to disk. Load the EM from this file again, by
   * calling this function with the path to the file. Optionally specify the node for the file
  ...
  /**
   * Returns a likelihood logarithm value and an index of the most probable mixture component for
   * the given sample.
   *
   * @param sample A sample for classification. It should be a one-channel matrix of \(1 \times
   *     dims\) or \(dims \times 1\) size.
   * @param probs Optional output matrix that contains posterior probabilities of each component
   *     given the sample. It has \(1 \times nclusters\) size and CV_64FC1 type.
   *     <p>The method returns a two-element double vector. Zero element is a likelihood logarithm
   *     value for the sample. First element is an index of the most probable mixture component for
   *     the given sample.
   * @return automatically generated
   */
  ...
  //
  // C++:  bool cv::ml::EM::trainE(Mat samples, Mat means0, Mat covs0 = Mat(), Mat weights0 = Mat(),
  // Mat& logLikelihoods = Mat(), Mat& labels = Mat(), Mat& probs = Mat())
  //

  /**
   * Estimate the Gaussian mixture parameters from a samples set.
   *
   * <p>This variation starts with Expectation step. You need to provide initial means \(a_k\) of
   ...
   */
  public boolean trainE(
      Mat samples, Mat means0, Mat covs0, Mat weights0, Mat logLikelihoods, Mat labels) {
    return trainE_1(
        nativeObj,
        samples.nativeObj,
        means0.nativeObj,
   ...
   *     further computing.
   * @param weights0 Initial weights \(\pi_k\) of mixture components. It should be a one-channel
   *     floating-point matrix with \(1 \times nclusters\) or \(nclusters \times 1\) size. each
   *     sample. It has \(nsamples \times 1\) size and CV_64FC1 type.
   *     \(\texttt{labels}_i=\texttt{arg max}_k(p_{i,k}), i=1..N\) (indices of the most probable
   *     mixture component for each sample). It has \(nsamples \times 1\) size and CV_32SC1 type.
   *     mixture component given the each sample. It has \(nsamples \times nclusters\) size and
   *     CV_64FC1 type.
  ...
  public boolean trainE(Mat samples, Mat means0, Mat covs0, Mat weights0) {
    return trainE_3(
        nativeObj, samples.nativeObj, means0.nativeObj, covs0.nativeObj, weights0.nativeObj);
  }
   ...
   *     covariance matrices is a one-channel matrix of \(dims \times dims\) size. If the matrices
   *     do not have CV_64F type they will be converted to the inner matrices of such type for the
   *     further computing. floating-point matrix with \(1 \times nclusters\) or \(nclusters \times
   *     1\) size. each sample. It has \(nsamples \times 1\) size and CV_64FC1 type.
   *     \(\texttt{labels}_i=\texttt{arg max}_k(p_{i,k}), i=1..N\) (indices of the most probable
   *     mixture component for each sample). It has \(nsamples \times 1\) size and CV_32SC1 type.
   *     mixture component given the each sample. It has \(nsamples \times nclusters\) size and
   *     CV_64FC1 type.
   ...
   * @param means0 Initial means \(a_k\) of mixture components. It is a one-channel matrix of
   *     \(nclusters \times dims\) size. If the matrix does not have CV_64F type it will be
   *     converted to the inner matrix of such type for the further computing. covariance matrices
   *     is a one-channel matrix of \(dims \times dims\) size. If the matrices do not have CV_64F
   *     type they will be converted to the inner matrices of such type for the further computing.
   *     floating-point matrix with \(1 \times nclusters\) or \(nclusters \times 1\) size. each
   *     sample. It has \(nsamples \times 1\) size and CV_64FC1 type.
   *     \(\texttt{labels}_i=\texttt{arg max}_k(p_{i,k}), i=1..N\) (indices of the most probable
   *     mixture component for each sample). It has \(nsamples \times 1\) size and CV_32SC1 type.
  ...
  //
  // C++:  bool cv::ml::EM::trainEM(Mat samples, Mat& logLikelihoods = Mat(), Mat& labels = Mat(),
  // Mat& probs = Mat())
  //
   ...
   * Estimate the Gaussian mixture parameters from a samples set.
   *
   * <p>This variation starts with Expectation step. Initial values of the model parameters will be
   * estimated by the k-means algorithm.
   *
   * <p>Unlike many of the ML models, %EM is an unsupervised learning algorithm and it does not take
   * responses (class labels or function values) as input. Instead, it computes the *Maximum
   * Likelihood Estimate* of the Gaussian mixture parameters from an input sample set, stores all
   * the parameters inside the structure: \(p_{i,k}\) in probs, \(a_k\) in means , \(S_k\) in
   * covs[k], \(\pi_k\) in weights , and optionally computes the output "class label" for each
   * sample: \(\texttt{labels}_i=\texttt{arg max}_k(p_{i,k}), i=1..N\) (indices of the most probable
   * mixture component for each sample).
   *
   * <p>The trained model can be used further for prediction, just like any other classifier. The
  ...
  public boolean trainEM(Mat samples, Mat logLikelihoods, Mat labels, Mat probs) {
    return trainEM_0(
        nativeObj, samples.nativeObj, logLikelihoods.nativeObj, labels.nativeObj, probs.nativeObj);
  }
   ...
   * @param samples Samples from which the Gaussian mixture model will be estimated. It should be a
   *     one-channel matrix, each row of which is a sample. If the matrix does not have CV_64F type
   *     it will be converted to the inner matrix of such type for the further computing. each
   *     sample. It has \(nsamples \times 1\) size and CV_64FC1 type.
   *     \(\texttt{labels}_i=\texttt{arg max}_k(p_{i,k}), i=1..N\) (indices of the most probable
   *     mixture component for each sample). It has \(nsamples \times 1\) size and CV_32SC1 type.
   *     mixture component given the each sample. It has \(nsamples \times nclusters\) size and
  ...
  //
  // C++:  bool cv::ml::EM::trainM(Mat samples, Mat probs0, Mat& logLikelihoods = Mat(), Mat& labels
  // = Mat(), Mat& probs = Mat())
  //
   ...
   * Estimate the Gaussian mixture parameters from a samples set.
   *
   * <p>This variation starts with Maximization step. You need to provide initial probabilities
  ...
  public boolean trainM(Mat samples, Mat probs0, Mat logLikelihoods, Mat labels) {
    return trainM_1(
        nativeObj, samples.nativeObj, probs0.nativeObj, logLikelihoods.nativeObj, labels.nativeObj);
  }
   ...
   *     one-channel matrix, each row of which is a sample. If the matrix does not have CV_64F type
   *     it will be converted to the inner matrix of such type for the further computing.
   * @param probs0 the probabilities each sample. It has \(nsamples \times 1\) size and CV_64FC1
   *     type. \(\texttt{labels}_i=\texttt{arg max}_k(p_{i,k}), i=1..N\) (indices of the most
   *     probable mixture component for each sample). It has \(nsamples \times 1\) size and CV_32SC1
   *     type. mixture component given the each sample. It has \(nsamples \times nclusters\) size
   *     and CV_64FC1 type.
   * @return automatically generated
   */
   ...
   * @param results The optional output \( nSamples \times nClusters\) matrix of results. It
   *     contains posterior probabilities for each sample from the input
   ...
   * Returns posterior probabilities for the provided samples
   *
   * @param samples The input samples, floating-point matrix posterior probabilities for each sample
   *     from the input
   * @return automatically generated
   */
   ...
   *
   * <p>Returns vector of covariation matrices. Number of matrices is the number of gaussian
   * mixtures, each matrix is a square floating-point matrix NxN, where N is the space
   * dimensionality.
   *
  ...
  // C++:  Vec2d cv::ml::EM::predict2(Mat sample, Mat& probs)
  private static native double[] predict2_0(
      long nativeObj, long sample_nativeObj, long probs_nativeObj);

  // C++:  bool cv::ml::EM::trainE(Mat samples, Mat means0, Mat covs0 = Mat(), Mat weights0 = Mat(),
  // Mat& logLikelihoods = Mat(), Mat& labels = Mat(), Mat& probs = Mat())
  private static native boolean trainE_0(
      long nativeObj,
      long samples_nativeObj,
      long means0_nativeObj,
      long covs0_nativeObj,
      long weights0_nativeObj,
      long logLikelihoods_nativeObj,
      long labels_nativeObj,
      long probs_nativeObj);

  private static native boolean trainE_1(
      long nativeObj,
      long samples_nativeObj,
      long means0_nativeObj,
      long covs0_nativeObj,
  ...
  private static native boolean trainE_2(
      long nativeObj,
      long samples_nativeObj,
      long means0_nativeObj,
      long covs0_nativeObj,
  ...
  private static native boolean trainE_3(
      long nativeObj,
      long samples_nativeObj,
      long means0_nativeObj,
      long covs0_nativeObj,
  ...
  private static native boolean trainE_4(
      long nativeObj, long samples_nativeObj, long means0_nativeObj, long covs0_nativeObj);

  private static native boolean trainE_5(
      long nativeObj, long samples_nativeObj, long means0_nativeObj);

  // C++:  bool cv::ml::EM::trainEM(Mat samples, Mat& logLikelihoods = Mat(), Mat& labels = Mat(),
  // Mat& probs = Mat())
  private static native boolean trainEM_0(
      long nativeObj,
      long samples_nativeObj,
      long logLikelihoods_nativeObj,
      long labels_nativeObj,
  ...
  private static native boolean trainEM_1(
      long nativeObj, long samples_nativeObj, long logLikelihoods_nativeObj, long labels_nativeObj);

  private static native boolean trainEM_2(
      long nativeObj, long samples_nativeObj, long logLikelihoods_nativeObj);

  private static native boolean trainEM_3(long nativeObj, long samples_nativeObj);

  // C++:  bool cv::ml::EM::trainM(Mat samples, Mat probs0, Mat& logLikelihoods = Mat(), Mat& labels
  // = Mat(), Mat& probs = Mat())
  private static native boolean trainM_0(
      long nativeObj,
      long samples_nativeObj,
  ...
  private static native boolean trainM_1(
      long nativeObj,
      long samples_nativeObj,
      long probs0_nativeObj,
      long logLikelihoods_nativeObj,
  ...
  private static native boolean trainM_2(
      long nativeObj, long samples_nativeObj, long probs0_nativeObj, long logLikelihoods_nativeObj);

  private static native boolean trainM_3(
      long nativeObj, long samples_nativeObj, long probs0_nativeObj);
  ...
  private static native void setTermCriteria_0(
      long nativeObj, int val_type, int val_maxCount, double val_epsilon);
