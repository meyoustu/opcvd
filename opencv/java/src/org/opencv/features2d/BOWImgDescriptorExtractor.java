 ...
 *
 * <p>Such a computation consists of the following steps:
 *
 * <p>1. Compute descriptors for a given image and its keypoints set. 2. Find the nearest visual
 * words from the vocabulary for each keypoint descriptor. 3. Compute the bag-of-words image
 * descriptor as is a normalized histogram of vocabulary words encountered in the image. The i-th
 * bin of the histogram is a frequency of i-th word of the vocabulary in the given image.
 */
public class BOWImgDescriptorExtractor {
  ...
  //
  // C++:   cv::BOWImgDescriptorExtractor::BOWImgDescriptorExtractor(Ptr_DescriptorExtractor
  // dextractor, Ptr_DescriptorMatcher dmatcher)
  //
  ...
  //
  // C++:  void cv::BOWImgDescriptorExtractor::compute2(Mat image, vector_KeyPoint keypoints, Mat&
  // imgDescriptor)
  //

  /**
   * @param imgDescriptor Computed output image descriptor. pointIdxsOfClusters[i] are keypoint
   *     indices that belong to the i -th cluster (word of vocabulary) returned if it is non-zero.
   ...
   *
   * @param vocabulary Vocabulary (can be trained using the inheritor of BOWTrainer ). Each row of
