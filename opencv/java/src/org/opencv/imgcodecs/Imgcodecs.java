...
public class Imgcodecs {
   ...
   * cannot be read (because of missing file, improper permissions, unsupported or invalid format),
   ...
   *   <li>On Microsoft Windows\* OS and MacOSX\*, the codecs shipped with an OpenCV image (libjpeg,
   *       libpng, libtiff, and libjasper) are used by default. So, OpenCV can always read JPEGs,
   *       PNGs, and TIFFs. On MacOSX, there is also an option to use native MacOSX image readers.
   *       But beware that currently these native image loaders give images with different pixel
   *       values because of the color management embedded into MacOSX.
   ...
   *       development files, for example, "libjpeg-dev", in Debian\* and Ubuntu\*) to get the codec
   ...
   *   <li>Use the IMREAD_UNCHANGED flag to keep the floating point values from PFM image.
   ...
   *       formats
   *   <li>32-bit float (CV_32F) images can be saved in PFM, TIFF, OpenEXR, and Radiance HDR
   *       formats; 3-channel (CV_32FC3) TIFF images will be saved using the LogLuv high dynamic
   ...
   *       transparent pixels should have alpha set to 0, fully opaque pixels should have alpha set
   *       to 255/65535 (see the code sample below).
   * </ul>
   *
   ...
   * XML or YAML format.
   *
   * <p>The sample below shows how to create a BGRA image and save it to a PNG file. It also
   * demonstrates how to set custom compression parameters: INCLUDE: snippets/imgcodecs_imwrite.cpp
   *
