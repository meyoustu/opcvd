...
public class KNearest extends StatModel {
   ...
   *
   * <p>The static method creates empty %KNearest classifier. It should be then trained using
   * StatModel::train method.
   *
   ...
   *
   * <p>Use KNearest::save to serialize and store an KNearest to disk. Load the KNearest from this
  ...
  //
  // C++:  float cv::ml::KNearest::findNearest(Mat samples, int k, Mat& results, Mat&
   ...
   *     sample. It is a single-precision floating-point vector with {@code
   ...
   *     <p>For each input vector (a row of the matrix samples), the method finds the k nearest
   *     neighbors. In case of regression, the predicted result is a mean value of the particular
   *     vector's neighbor responses. In case of classification, the class is determined by voting.
   *     <p>For each input vector, the neighbors are sorted by their distances to the vector.
   *     <p>In case of C++ interface you can use output pointers to empty matrices and the function
   *     will allocate memory itself.
   *     <p>If only a single input vector is passed, all output matrices are optional and the
   *     predicted value is returned by the method.
   ...
   *     &lt;number_of_samples&gt;} elements.
   * @param neighborResponses Optional output values for corresponding neighbors. It is a single-
   *     precision floating-point matrix of {@code &lt;number_of_samples&gt; * k} size. is a
   *     single-precision floating-point matrix of {@code &lt;number_of_samples&gt; * k} size.
   *     <p>For each input vector (a row of the matrix samples), the method finds the k nearest
   *     neighbors. In case of regression, the predicted result is a mean value of the particular
   *     vector's neighbor responses. In case of classification, the class is determined by voting.
   *     <p>For each input vector, the neighbors are sorted by their distances to the vector.
  ...
  public float findNearest(Mat samples, int k, Mat results, Mat neighborResponses) {
    return findNearest_1(
        nativeObj, samples.nativeObj, k, results.nativeObj, neighborResponses.nativeObj);
  }
   ...
   * @param results Vector with results of prediction (regression or classification) for each input
   *     sample. It is a single-precision floating-point vector with {@code
   *     &lt;number_of_samples&gt;} elements. precision floating-point matrix of {@code
   *     &lt;number_of_samples&gt; * k} size. is a single-precision floating-point matrix of {@code
  ...
  // C++:  float cv::ml::KNearest::findNearest(Mat samples, int k, Mat& results, Mat&
  // neighborResponses = Mat(), Mat& dist = Mat())
  private static native float findNearest_0(
      long nativeObj,
      long samples_nativeObj,
      int k,
      long results_nativeObj,
      long neighborResponses_nativeObj,
      long dist_nativeObj);

  private static native float findNearest_1(
      long nativeObj,
      long samples_nativeObj,
      int k,
      long results_nativeObj,
  ...
  private static native float findNearest_2(
      long nativeObj, long samples_nativeObj, int k, long results_nativeObj);
