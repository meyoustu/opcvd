 ...
 *
 * <p>Neural network is presented as directed acyclic graph (DAG), where vertices are Layer
 * instances, and edges specify relationships between layers inputs and outputs.
 *
 * <p>Each network layer has unique integer id and unique string name inside its network. LayerId
 * can store either layer name or layer id.
 *
 * <p>This class supports reference counting of its instances, i. e. copies point to the same
 * instance.
 */
public class Net {
   ...
   * @return blob for first output of specified layer. By default runs forward pass for the whole
   *     network.
   */
   ...
   * Returns parameter blob of the layer.
   *
   * @param layer name or id of the layer. SEE: Layer::blobs
   * @return automatically generated
   */
   ...
   * @param bin Binary file with trained weights. Networks imported from Intel's Model Optimizer are
   *     launched in Intel's Inference Engine backend.
   * @return automatically generated
   */
  ...
  //
  // C++: static Net cv::dnn::Net::readFromModelOptimizer(vector_uchar bufferModelConfig,
  // vector_uchar bufferWeights)
  //

  /**
   * Create a network from Intel's Model Optimizer in-memory buffers with intermediate
   * representation (IR).
   *
  ...
  public static Net readFromModelOptimizer(MatOfByte bufferModelConfig, MatOfByte bufferWeights) {
    ...
    return new Net(
        readFromModelOptimizer_1(bufferModelConfig_mat.nativeObj, bufferWeights_mat.nativeObj));
  }
   ...
   * Dump net to String
   *
   * @return String with structure, hyperparameters, backend, target and fusion Call method after
   *     setInput(). To see correct backend, target and fusion run after forward().
   */
  public String dump() {
  ...
  /**
   * Returns overall time for inference and timings (in ticks) for layers. Indexes in returned
   * vector correspond to layers ids. Some layers can be fused with others, in this case zero ticks
   * count will be return for that skipped layers.
   *
   ...
   *     <p>Descriptors have the following template
   *     &lt;DFN&gt;&amp;lt;layer_name&amp;gt;[.input_number]&lt;/DFN&gt;: - the first part of the
   *     template &lt;DFN&gt;layer_name&lt;/DFN&gt; is string name of the added layer. If this part
   *     is empty then the network input pseudo layer will be used; - the second optional part of
   *     the template &lt;DFN&gt;input_number&lt;/DFN&gt; is either number of the layer input,
   *     either label one. If this part is omitted then the first layer input will be used.
   *     <p>SEE: setNetInputs(), Layer::inputNameToIndex(), Layer::outputNameToIndex()
   */
   ...
   * @param outputName name for layer which output is needed to get If {@code outputName} is empty,
   ...
   * Runs forward pass to compute output of layer with name {@code outputName}.
   *
   * @param outputBlobs contains all output blobs for specified layer. If {@code outputName} is
   *     empty, runs forward pass for the whole network.
   */
  ...
  //
  // C++:  void cv::dnn::Net::getLayersShapes(MatShape netInputShape, vector_int& layersIds,
  // vector_vector_MatShape& inLayersShapes, vector_vector_MatShape& outLayersShapes)
  //

  // Unknown type 'vector_vector_MatShape' (O), skipping the function

  //
  // C++:  void cv::dnn::Net::getLayersShapes(vector_MatShape netInputShapes, vector_int& layersIds,
  // vector_vector_MatShape& inLayersShapes, vector_vector_MatShape& outLayersShapes)
  //
  ...
  //
  // C++:  void cv::dnn::Net::getMemoryConsumption(MatShape netInputShape, size_t& weights, size_t&
  // blobs)
  //
  ...
  //
  // C++:  void cv::dnn::Net::getMemoryConsumption(int layerId, MatShape netInputShape, size_t&
  // weights, size_t& blobs)
  //

  public void getMemoryConsumption(
      int layerId, MatOfInt netInputShape, long[] weights, long[] blobs) {
    Mat netInputShape_mat = netInputShape;
    double[] weights_out = new double[1];
    double[] blobs_out = new double[1];
  ...
  //
  // C++:  void cv::dnn::Net::getMemoryConsumption(int layerId, vector_MatShape netInputShapes,
  // size_t& weights, size_t& blobs)
  //

  public void getMemoryConsumption(
      int layerId, List<MatOfInt> netInputShapes, long[] weights, long[] blobs) {
    double[] weights_out = new double[1];
    double[] blobs_out = new double[1];
   ...
   *
   * @param scheduler Path to YAML file with scheduling directives. SEE: setPreferableBackend
   *     <p>Schedule layers that support Halide backend. Then compile them for specific target. For
   *     layers that not represented in scheduling file or if no manual scheduling used at all,
  ...
  //
  // C++:  void cv::dnn::Net::setInput(Mat blob, String name = "", double scalefactor = 1.0, Scalar
  // mean = Scalar())
  //
   ...
   * @param mean An optional mean subtraction values. SEE: connect(String, String) to know format of
   *     the descriptor.
   *     <p>If scale or mean values are specified, a final input blob is computed as:
   ...
   * @param blob A new blob. Should have CV_32F or CV_8U depth.
   * @param name A name of input layer.
   * @param scalefactor An optional normalization scale. SEE: connect(String, String) to know format
   *     of the descriptor.
   *     <p>If scale or mean values are specified, a final input blob is computed as:
   *     \(input(n,c,h,w) = scalefactor \times (blob(n,c,h,w) - mean_c)\)
   */
   ...
   *
   * @param blob A new blob. Should have CV_32F or CV_8U depth.
   * @param name A name of input layer. SEE: connect(String, String) to know format of the
   *     descriptor.
   *     <p>If scale or mean values are specified, a final input blob is computed as:
   *     \(input(n,c,h,w) = scalefactor \times (blob(n,c,h,w) - mean_c)\)
   */
   ...
   * Sets the new input value for the network
   *
   * @param blob A new blob. Should have CV_32F or CV_8U depth. SEE: connect(String, String) to know
   ...
   *
   * <p>Each net always has special own the network input pseudo layer with id=0. This layer stores
   * the user blobs only and don't make any computations. In fact, this layer provides the only way
   * to pass user data into the network. As any other layer, this layer can label its outputs and
   * this function provides an easy way to do this.
   *
   ...
   * @param blob the new value. SEE: Layer::blobs <b>Note:</b> If shape of the new blob differs from
   *     the previous shape, then the following forward pass may fail.
   */
   ...
   *
   * @param backendId backend identifier. SEE: Backend
   *     <p>If OpenCV is compiled with Intel's Inference Engine library, DNN_BACKEND_DEFAULT means
   *     DNN_BACKEND_INFERENCE_ENGINE. Otherwise it equals to DNN_BACKEND_OPENCV.
   */
   ...
   *
   * @param targetId target identifier. SEE: Target
   *     <p>List of supported combinations backend / target: | | DNN_BACKEND_OPENCV |
   *     DNN_BACKEND_INFERENCE_ENGINE | DNN_BACKEND_HALIDE | DNN_BACKEND_CUDA |
   *     |------------------------|--------------------|------------------------------|--------------------|-------------------|
   *     | DNN_TARGET_CPU | + | + | + | | | DNN_TARGET_OPENCL | + | + | + | | |
   *     DNN_TARGET_OPENCL_FP16 | + | + | | | | DNN_TARGET_MYRIAD | | + | | | | DNN_TARGET_FPGA | |
  ...
  // C++: static Net cv::dnn::Net::readFromModelOptimizer(vector_uchar bufferModelConfig,
  // vector_uchar bufferWeights)
  private static native long readFromModelOptimizer_1(
  ...
  // C++:  int64 cv::dnn::Net::getFLOPS(int layerId, MatShape netInputShape)
  private static native long getFLOPS_1(
      long nativeObj, int layerId, long netInputShape_mat_nativeObj);
  ...
  // C++:  void cv::dnn::Net::forward(vector_Mat& outputBlobs, String outputName = String())
  private static native void forward_2(
      long nativeObj, long outputBlobs_mat_nativeObj, String outputName);

  private static native void forward_3(long nativeObj, long outputBlobs_mat_nativeObj);

  // C++:  void cv::dnn::Net::forward(vector_Mat& outputBlobs, vector_String outBlobNames)
  private static native void forward_4(
      long nativeObj, long outputBlobs_mat_nativeObj, List<String> outBlobNames);
  ...
  // C++:  void cv::dnn::Net::getMemoryConsumption(MatShape netInputShape, size_t& weights, size_t&
  // blobs)
  private static native void getMemoryConsumption_0(
      long nativeObj, long netInputShape_mat_nativeObj, double[] weights_out, double[] blobs_out);

  // C++:  void cv::dnn::Net::getMemoryConsumption(int layerId, MatShape netInputShape, size_t&
  // weights, size_t& blobs)
  private static native void getMemoryConsumption_1(
      long nativeObj,
      int layerId,
      long netInputShape_mat_nativeObj,
      double[] weights_out,
      double[] blobs_out);

  // C++:  void cv::dnn::Net::getMemoryConsumption(int layerId, vector_MatShape netInputShapes,
  // size_t& weights, size_t& blobs)
  private static native void getMemoryConsumption_2(
      long nativeObj,
      int layerId,
  ...
  // C++:  void cv::dnn::Net::setInput(Mat blob, String name = "", double scalefactor = 1.0, Scalar
  // mean = Scalar())
  private static native void setInput_0(
      long nativeObj,
      long blob_nativeObj,
      String name,
      double scalefactor,
  ...
  private static native void setInput_1(
      long nativeObj, long blob_nativeObj, String name, double scalefactor);
  ...
  // C++:  void cv::dnn::Net::setInputShape(String inputName, MatShape shape)
  private static native void setInputShape_0(
      long nativeObj, String inputName, long shape_mat_nativeObj);

  // C++:  void cv::dnn::Net::setInputsNames(vector_String inputBlobNames)
  private static native void setInputsNames_0(long nativeObj, List<String> inputBlobNames);

  // C++:  void cv::dnn::Net::setParam(LayerId layer, int numParam, Mat blob)
  private static native void setParam_0(
      long nativeObj, long layer_nativeObj, int numParam, long blob_nativeObj);
