...
public class TrainData {
   ...
   *
   * <p>In current implementation the function tries to avoid physical data copying and returns the
  ...
  //
  // C++: static Ptr_TrainData cv::ml::TrainData::create(Mat samples, int layout, Mat responses, Mat
  // varIdx = Mat(), Mat sampleIdx = Mat(), Mat sampleWeights = Mat(), Mat varType = Mat())
  //
   ...
   * @param sampleIdx vector specifying which samples to use for training. It can be an integer
   *     vector (CV_32S) containing 0-based sample indices or byte vector (CV_8U) containing a mask
   *     of training samples. &lt;number_of_variables_in_responses&gt;`, containing types of each
   *     input and output variable. See ml::VariableTypes.
   * @return automatically generated
   */
  public static TrainData create(
    ...
    return TrainData.__fromPtr__(
        create_2(
            samples.nativeObj, layout, responses.nativeObj, varIdx.nativeObj, sampleIdx.nativeObj));
  }
   ...
   * @param varIdx vector specifying which variables to use for training. It can be an integer
   *     vector (CV_32S) containing 0-based variable indices or byte vector (CV_8U) containing a
   *     mask of active variables. vector (CV_32S) containing 0-based sample indices or byte vector
   *     (CV_8U) containing a mask of training samples. &lt;number_of_variables_in_responses&gt;`,
   ...
   *     single row or as a single column. The matrix should have type CV_32F or CV_32S (in the
   *     former case the responses are considered as ordered by default; in the latter case - as
   *     categorical) (CV_32S) containing 0-based variable indices or byte vector (CV_8U) containing
   *     a mask of active variables. vector (CV_32S) containing 0-based sample indices or byte
   ...
   *
   * <p>The function selects a subset of specified relative size and then returns it as the training
   * set. If the function is not called, all the data is used for training. Please, note that for
   * each of TrainData::getTrain\* there is corresponding TrainData::getTest\*, so that the test
   * subset can be retrieved and processed as well. SEE: TrainData::setTrainTestSplit
   *
